 ...
 * Templated on activation dtype T ∈ {fp16, bf16}. Internal accumulation fp32.
 */
...
struct FusedAddRmsNorm_batched_kernel {
    T*       hidden_ptr;    // [rows, K] — input and output
    T*       residual_ptr;  // [rows, K] — updated in-place
    ...
    void operator()(sycl::nd_item<1> item) const SYCL_ESIMD_KERNEL {
        ...
        for (int c = 0; c < n_chunks; c++) {
            int offset = base + c * VL;
            simd<float, VL> r = block_load<T, VL>(residual_ptr + offset);
            simd<float, VL> w = block_load<T, VL>(weight_ptr + c * VL);
...
template <typename T>
inline void fused_add_rms_norm_batched_host_t(
    ...
    int rows, int K, float eps, sycl::queue& q)
{
    q.submit([&](sycl::handler& cgh) {
        cgh.parallel_for(
            ...
            FusedAddRmsNorm_batched_kernel<T>{
...
inline void fused_add_rms_norm_batched_host(
    ...
    int rows, int K, float eps, sycl::queue& q)
{
    fused_add_rms_norm_batched_host_t<fp16>(
        hidden_ptr, residual_ptr, weight_ptr, rows, K, eps, q);
}
